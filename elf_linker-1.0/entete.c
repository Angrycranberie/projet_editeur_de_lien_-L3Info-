...
// et retourne la structure d'en-tête remplie.
...
Elf_identificator Lecture_identificateur_vocal(FILE *f)
{
  ...
  // extension. (cette partie et la suivante sont sans doute sans improtance, mais permettent
  ...
  return header;
  // Saute le reste de l'identification, qui est vide
}
...
Elf32_Ehdr Lecture32(FILE *f, Elf_identificator headid)
{
  ...
  // Récupère ce qui a déjà été lu et passé en argument
  for (i = 0; i < EI_NIDENT; i++)
  {
...
Elf32_Ehdr Lecture32_vocal(FILE *f, Elf_identificator headid)
{
  ...
  // Lit et verifie le type du fichier elf
  fread(&(header.e_type), sizeof(Elf32_Half), 1, f);
  ...
  // Lit et vérifie quel est le type de machine
  fread(&(header.e_machine), sizeof(Elf32_Half), 1, f);
  switch (header.e_machine)
  // Problème: il y a beaucoup de machines différentes dans elf.h ; le switch a donc été réalisé directement à partir de elf.h, grâce à des 'replace'.
  {
  case 0:
