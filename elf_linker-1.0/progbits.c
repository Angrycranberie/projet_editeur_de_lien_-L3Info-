...
Merge_table_progbits search_progbits_f2_64(FILE* f1, FILE* f2, Elf64_Ehdr header1, Elf64_Ehdr header2){
  ...
  // On crée la table 
  Merge_table_progbits Mtable = initmerge(f2,header1,header2);
...
// Imprime le header dans le fichier destination.
void printheader_64(FILE* f, Elf64_Ehdr header){
  fwrite(&header.e_ident, sizeof(unsigned char), EI_NIDENT, f);
  fwrite(&header.e_type, sizeof(Elf64_Half), 1, f);
  fwrite(&header.e_machine, sizeof(Elf64_Half), 1, f);
  fwrite(&header.e_version, sizeof(Elf64_Word), 1, f);
  fwrite(&header.e_entry, sizeof(Elf64_Addr), 1, f);
  fwrite(&header.e_phoff, sizeof(Elf64_Off), 1, f);
  fwrite(&header.e_shoff, sizeof(Elf64_Off), 1, f);
  fwrite(&header.e_flags, sizeof(Elf64_Word), 1, f);
  fwrite(&header.e_ehsize, sizeof(Elf64_Half), 1, f);
  fwrite(&header.e_phentsize, sizeof(Elf64_Half), 1, f);
  fwrite(&header.e_phnum, sizeof(Elf64_Half), 1, f);
  fwrite(&header.e_shentsize, sizeof(Elf64_Half), 1, f);
  fwrite(&header.e_shnum, sizeof(Elf64_Half), 1, f);
  fwrite(&header.e_shstrndx, sizeof(Elf64_Half), 1, f);
}

// Imprime l'entrée de la table des sections dans le fichier destination.
void printsection_64(FILE* f, Elf64_Shdr section){
  fwrite(&section.sh_name, sizeof(Elf64_Word), 1, f);
  fwrite(&section.sh_type, sizeof(Elf64_Word), 1, f);
  fwrite(&section.sh_flags, sizeof(Elf64_Xword), 1, f);
  fwrite(&section.sh_addr, sizeof(Elf64_Addr), 1, f);
  fwrite(&section.sh_offset, sizeof(Elf64_Off), 1, f);
  fwrite(&section.sh_size, sizeof(Elf64_Xword), 1, f);
  fwrite(&section.sh_link, sizeof(Elf64_Word), 1, f);
  fwrite(&section.sh_info, sizeof(Elf64_Word), 1, f);
  fwrite(&section.sh_addralign, sizeof(Elf64_Xword), 1, f);
  fwrite(&section.sh_entsize, sizeof(Elf64_Xword), 1, f);
}
...
Elf64_Section_progbits init_section_size(int taille){
  Elf64_Section_progbits Section;
...
Table_sections_64 get_merged_progbits_64 (FILE* f1, FILE* f2, Elf64_Ehdr header1, Elf64_Ehdr header2, Merge_table_progbits Mtable){
  ...
  for (i = 1; i<header1.e_shnum; i++){
    ...
    if (type == SHT_PROGBITS){
      ...
      verif = verif_fusion_progbits(i, Mtable, header2);
      // Cas de la fusion 
      if (verif != -1 ){  
