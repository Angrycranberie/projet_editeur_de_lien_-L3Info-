...
/* Fonction de lecture d'une section d'un fichier ELF.
...
section_list *read_tablesection(FILE *f, Elf64_Ehdr header)
{
	...
	// Initialisation de la structure qui regroupe toutes les informations de chaque section du fichier et leur nom associé.
	...
	// Version : 0 pour 32 bits, 1 pour 64 bits.
    int bits_version = header.e_ident[4] == ELFCLASS64;
    // Indique si le boutisme est différent entre la machine et le fichier (1 pour 'oui', 0 pour 'non').
    int diff_endianess = (is_big_endian() != (int)(header.e_ident[EI_DATA] == ELFDATA2MSB));

	if (header.e_shoff == 0) return NULL;

	// On se place au début de la table des sections ; on fait une entrée pour chaque section.
	for (i = 0; i < sections->nb_section; i++){
		// Se place à l'emplacement du fichier donné par l'offset et le numéro de la section dans la table.
...
/* Fonction d'affichage d'une table des sections.
...
void print_sectionTable(section_list *sections){
	...
	for (i = 0; i < sections->nb_section; i++){
		...
		printf("FLAGS:\n");
		if (sections->sec_list[i].sh_flags & SHF_WRITE)	printf("  Writable\n");
		if (sections->sec_list[i].sh_flags & SHF_ALLOC)	printf("  Occupies memory during execution\n");
		if (sections->sec_list[i].sh_flags & SHF_EXECINSTR)	printf("  Executable\n");
		if (sections->sec_list[i].sh_flags & SHF_MERGE)	printf("  Might be merged\n");
		if (sections->sec_list[i].sh_flags & SHF_STRINGS) printf("  Contains nul-terminated strings\n");
		if (sections->sec_list[i].sh_flags & SHF_INFO_LINK)	printf("  sh_info' contains SHT index\n");
		if (sections->sec_list[i].sh_flags & SHF_LINK_ORDER) printf("  Preserve order after combining\n");
		if (sections->sec_list[i].sh_flags & SHF_OS_NONCONFORMING) printf("  Non-standard OS specific handling required\n");
		if (sections->sec_list[i].sh_flags & SHF_GROUP)	printf("  Section is member of a group\n");
		if (sections->sec_list[i].sh_flags & SHF_TLS) printf("  Section hold thread-local data\n");
		if (sections->sec_list[i].sh_flags & SHF_COMPRESSED) printf("  Section with compressed data\n");
