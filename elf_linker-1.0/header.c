...
/* Fonction de lecture de l'entête d'un fichier ELF.
...
Elf64_Ehdr read_header(FILE *f)
{
    ...
    {
        fprintf(stderr, "  boutisme invalide\n");
        return header;
    }
    ...
    if (diff_endianess)
    {
        ...
        header.e_entry = bits_version ? reverse_8(header.e_entry) : reverse_4(header.e_entry);
        header.e_phoff = bits_version ? reverse_8(header.e_phoff) : reverse_4(header.e_phoff);
        header.e_shoff = bits_version ? reverse_8(header.e_shoff) : reverse_4(header.e_shoff);
...
void print_header(Elf64_Ehdr header)
{
    ...
    /* Vérifie l'OS utilisé (on ne connait pas les autres codes, mais 0 signifie Linux sans extension.
    Cette partie et la suivante sont sans doute sans importance, mais permettent d'imiter l'affichage de
    la commande 'readelf -h'. */
    ...
    /* Il y a beaucoup de machines différentes dans 'elf.h'.
    Le switch a donc été réalisé directement à partir de ce fichier à l'aide de remplacements. */
