...
void read_rela_tables_32(int id, FILE* f, Elf32_Ehdr header, int addend){
  ...
  fread(&linksym,sizeof(Elf32_Word),1,f);
  ...
  for (i=0;i<nb;i++){
    ...
    // On lit l'offset
    fread(&rela_entry.r_offset,sizeof(Elf32_Addr),1,f);
    
    // On lit les informations (type et index du symbole)
    fread(&rela_entry.r_info,sizeof(Elf32_Word),1,f);
    
    // On lit l'addend si necessaire
    if (addend==1) fread(&rela_entry.r_addend,sizeof(Elf32_Sword),1,f);
    ...
    // Afficher le type: Attention, on ne va donner ici que le numero lu, car les methodes s'ensuivant varient selon le processeur
    type = ELF32_R_TYPE(rela_entry.r_info);
    printf("%5d            ",type); 
    
    //Afficher l'index du symbole
    sym = ELF32_R_SYM(rela_entry.r_info);
    printf("%5d            ",sym); 
    ...
    fread(&offset,sizeof(Elf32_Off),1,f);
    fseek(f, offset +  sym + sizeof(Elf32_Word) , SEEK_SET);
...
void search_reloca_tables_32(FILE* f, Elf32_Ehdr header){
  ...
  for (i=0;i<header.e_shnum;i=i+1){
    ...
    // On separe les cas avec ou sans addends
    if (type == SHT_RELA ) read_rela_tables_32(i,f,header,1);
    else if (type == SHT_REL ) read_rela_tables_32(i,f,header,0);
  }
}
